document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderAudiences() {
      ...
      if (!localStorage.getItem("initialDataLoaded")) {
        const sampleAudiences = ["101", "102", "201", "Лаборатория 1", "Лаборатория 2"];
        const sampleGroups = ["ИС-101", "ПМИ-201", "ПО-301"];
        const sampleUsers = {
          "ivanov": { password: "1234", role: "student" },
          "petrova": { password: "1234", role: "teacher" },
          "admin": { password: "admin", role: "admin" }
        };
        const sampleComputers = {};
    
        sampleAudiences.forEach(aud => {
          sampleComputers[aud] = [];
          const count = Math.floor(Math.random() * 4) + 2; 
          for (let i = 1; i <= count; i++) {
            sampleComputers[aud].push({
              name: `${aud}-PC-${i}`,
              status: Math.random() < 0.8 ? "working" : "broken",
              reason: "Технический сбой",
              internet: Math.random() < 0.9
            });
          }
        });
    
        const sampleSchedule = [
          { group: "ИС-101", auditory: "101", time: "08:30 - 10:00", teacher: "petrova" },
          { group: "ПМИ-201", auditory: "201", time: "10:15 - 11:45", teacher: "petrova" },
          { group: "ПО-301", auditory: "Лаборатория 1", time: "12:00 - 13:30", teacher: "petrova" }
        ];
    
        localStorage.setItem("audiences", JSON.stringify(sampleAudiences));
        localStorage.setItem("groups", JSON.stringify(sampleGroups));
        localStorage.setItem("users", JSON.stringify(sampleUsers));
        localStorage.setItem("computers", JSON.stringify(sampleComputers));
